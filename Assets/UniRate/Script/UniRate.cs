...
public class UniRate : MonoBehaviour {
	...
	/// <summary>
	/// If true, only add use count when a fresh start. 
	/// Otherwise the use count will be increased even the app switched from background. 
	/// Default is false.
	/// </summary>
	public bool onlyCountUseWhenFreshStartUp = false;
	...
	void OnApplicationPause(bool pauseStatus) {
        if (!pauseStatus && _instance != null && !this.onlyCountUseWhenFreshStartUp) {
			IncreaseUseCount();
	...
	void CheckAndReadyToRate(bool showPrompt) {
		...
		if (!_currentChecking) {
			...
			#elif UNITY_ANDROID || UNITY_WP8 || UNITY_WSA
			if (showPrompt) {
				ReadyToPrompt();
			} else {
				OpenRatePage();
...
public class UniRateDebug {
	public static void Log(object message) {
        ...
        #if UniRateDebug && (UNITY_WP8 || UNITY_WSA)
